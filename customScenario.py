...
class Scenario(BaseScenario):
    ...
    def reset_world_at(self, env_index: int = None):
        ...
        for _ in self.world.agents:
            position = ScenarioUtils.find_random_pos_for_entity(
                ...
                x_bounds=(-self.world_semidim*0.6, self.world_semidim*0.6),
                y_bounds=(-self.world_semidim*0.6, self.world_semidim*0.6),
            )
    ...
    def reward(self, agent: Agent):
        ...
        #     if other_agent != agent and self.world.get_distance(agent, other_agent) <= self.lidar_range:
        #         no_agents_in_sight = False
        #         break

        # # Apply a bonus reward for moving directly to the goal when no other agents are in sight
        # if no_agents_in_sight:
        #     direct_movement_bonus = 0.1 * torch.linalg.vector_norm(agent.state.pos - agent.goal.state.pos, dim=-1)
        #     agent.pos_rew += direct_movement_bonus
